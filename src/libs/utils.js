...
export async function sign(
  ...
  return client.signAmino2(
    device.startsWith("ledger") ? toSignAddress(signerAddress) : signerAddress,
    messages,
    fee,
    memo,
    signerData
...
export async function getLedgerAddress(
  ...
  const ledgerName = getLedgerAppName(coinType);
  const signer = new LedgerSigner(trans, {
