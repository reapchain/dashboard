...
import { reapToEth } from "@/libs/metamask/addressConverter";
...
export default {
  ...
  computed: {
    defaultWallet() {
      return this.$store?.state?.chains?.defaultWallet || "Wallet";
    },
    calculateTotal() {
      let total = 0;
  ...
  mounted() {},
  methods: {
    convertToEthAddress(address) {
      return reapToEth(address);
    },
    refreshPrice() {
      ...
      if (this.accounts) {
        Object.keys(this.accounts).forEach((acc) => {
          this.accounts[acc].address.forEach((add) => {
            this.$http
              .getBankBalances(add.addr, chains[add.chain])
              .then((res) => {
                if (res && res.length > 0) {
                  ...
                  res.forEach((token) => {
                    if (token.denom.startsWith("ibc")) {
                      this.$http
                        .getIBCDenomTrace(token.denom, chains[add.chain])
                        .then((denom) => {
            ...
            this.$http
              .getStakingDelegations(add.addr, chains[add.chain])
              .then((res) => {
                if (res.delegation_responses) {
                  const delegation = res.delegation_responses
                    .map((x) => x.balance)
                    .reduce((t, c) => {
                      const t1 = t;
                      if (t1[c.denom]) {
    ...
    formatBalance(v) {
      ...
      if (balance) {
        const ret = balance
          .map((x) => this.formatCurrency(x.amount, x.denom))
          .reduce((t, c) => t + c);
        total += ret;
      }
      const delegations = this.delegations[v];
      if (delegations) {
        const ret = delegations
          .map((x) => this.formatCurrency(x.amount, x.denom))
          .reduce((t, c) => t + c, 0);
        total += ret;
      }
    ...
    formatBalanceChanges(v) {
      ...
      if (balance) {
        const ret = balance
          .map(
            (x) =>
              this.formatCurrency(x.amount, x.denom) *
              this.getChanges(x.denom) *
              0.01
          )
          .reduce((t, c) => t + c);
        total += ret;
    ...
    removeAddress(v) {
      ...
      const remainAccounts = Object.keys(this.accounts);
      if (remainAccounts && remainAccounts.length > 0) {
        this.updateDefaultWallet(remainAccounts[0]);
      } else {
