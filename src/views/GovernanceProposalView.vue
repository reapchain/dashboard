<template>
  <section>
        ...
        <b-table-simple stacked="sm" hover striped>
          <tbody>
            <b-tr>
              ...
              >
                {{ $t("governanceProposal.proposal_total_deposit") }} </b-td
    ...
    <b-card no-body>
      ...
      <b-card-body>
        <div>
          <!-- totalPower {{ totalPower }} proposal.tally.total
          {{ proposal.tally.total }}
          proposal.status {{ proposal.status }} 100 * (totalPower /
          proposal.tally.total) ->
          {{ 100 * (totalPower / proposal.tally.total) }} -->
          <div class="scale">
            <div class="box">
              <b-progress
                :max="
                  totalPower && proposal.status === 2
                    ? 100 * (totalPower / proposal.tally.total)
                    : 100
                "
                height="2rem"
              ...
              <b-tooltip :target="'vote-yes' + proposal.id">
                {{ percent(proposal.tally.yes) }}%
                {{ $t("governanceProposal.proposal_votes_yes") }}
              </b-tooltip>
              <b-tooltip :target="'vote-no' + proposal.id">
                {{ percent(proposal.tally.no) }}%
                {{ $t("governanceProposal.proposal_votes_no") }}
              </b-tooltip>
              <b-tooltip :target="'vote-veto' + proposal.id">
                {{ percent(proposal.tally.veto) }}% voted
                {{ $t("governanceProposal.proposal_votes_nwv") }}
              </b-tooltip>
              <b-tooltip :target="'vote-abstain' + proposal.id">
                {{ percent(proposal.tally.abstain) }}%
                {{ $t("governanceProposal.proposal_votes_abstain") }}
              </b-tooltip>
    ...
    <b-card v-if="proposal.total_deposit" no-body>
      <b-card-header>
        <b-card-title>
          {{ $t("governanceProposal.proposal_deposits") }} ({{
  ...
  computed: {
    upgradeTime() {
      if (this.proposal.type.indexOf("SoftwareUpgrade") > 0) {
        if (
          Number(this.proposal?.contents.plan.height || 0) > 0 &&
          this.latest?.block
        ) {
          const blocks =
            Number(this.proposal.contents.plan.height) -
            Number(this.latest.block?.header?.height || 0);
          if (blocks > 0) {
            const endtime = dayjs()
              .add(blocks * this.blocktime, "second")
              .format("YYYY-MM-DD HH:mm:ss");
            return endtime;
          }
        }
        return dayjs(this.proposal.contents.plan.time).format(
          "YYYY-MM-DD HH:mm:ss"
        );
      }
  ...
  methods: {
    ...
    scaleWidth(p) {
      if (this.tallyParam) {
        if (p.status === 2) {
          return (
            Number(this.tallyParam.quorum) *
            Number(this.tallyParam.threshold) *
            (1 - p.tally.abstain) *
            100
          );
        }
